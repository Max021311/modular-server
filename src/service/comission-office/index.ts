import type {
  ComissionOfficeServiceI,
  FindAndCountParams,
  CreateComissionOffice,
  UpdateComissionOffice
} from './types.js'
...
type ComissionOffice = Pick<Tables['ComissionOffices']['base'], 'id'|'studentId'|'vacancyId'|'cycleId'|'beginDate'|'status'|'fileId'|'createdAt'|'updatedAt'>
...
type ComissionOfficeWithJoins = ComissionOffice & AtLeastOneJoin<[CycleJoin, StudentJoin, VacancyJoin]>

export class ComissionOfficeService implements ComissionOfficeServiceI {
  ...
  private get selectQuery () {
    const db = this.db
    return db.table('ComissionOffices')
      .select<ComissionOfficeWithJoins[]>(
        db.ref('id').withSchema('ComissionOffices'),
        db.ref('studentId').withSchema('ComissionOffices'),
        db.ref('vacancyId').withSchema('ComissionOffices'),
        db.ref('cycleId').withSchema('ComissionOffices'),
        db.ref('beginDate').withSchema('ComissionOffices'),
        db.ref('status').withSchema('ComissionOffices'),
        db.ref('fileId').withSchema('ComissionOffices'),
        db.ref('createdAt').withSchema('ComissionOffices'),
        db.ref('updatedAt').withSchema('ComissionOffices')
      )
  }

  private applyCycleJoin <T extends Knex.QueryBuilder> (query: T, db: Knex) {
    return query.leftJoin('Cycles', 'ComissionOffices.cycleId', '=', 'Cycles.id')
      .select(
  ...
  private applyStudentJoin <T extends Knex.QueryBuilder> (query: T, db: Knex) {
    return query.leftJoin('Students', 'ComissionOffices.studentId', '=', 'Students.id')
      .select(
  ...
  private applyVacancyJoin <T extends Knex.QueryBuilder> (query: T, db: Knex) {
    return query.leftJoin('Vacancies', 'ComissionOffices.vacancyId', '=', 'Vacancies.id')
      .select(
  ...
  async getById (id: number) {
    const result = await this.selectQuery.where('ComissionOffices.id', '=', id).first()
  ...
  async findAndCount (params: FindAndCountParams) {
    ...
    const db = this.db
    let countQuery = db.table('ComissionOffices')
      .count({ count: '*' }).first()
    let selectQuery = this.selectQuery
      .limit(limit)
      .offset(offset)

    if (order) selectQuery = selectQuery.orderBy([{ column: order[0], order: order[1] }, { column: 'ComissionOffices.id', order: order[1] }])

    if (cycleId !== undefined) {
      selectQuery = selectQuery.where('ComissionOffices.cycleId', '=', cycleId)
      countQuery = countQuery.where('cycleId', '=', cycleId)
    }
    if (studentId !== undefined) {
      selectQuery = selectQuery.where('ComissionOffices.studentId', '=', studentId)
      countQuery = countQuery.where('studentId', '=', studentId)
    }
    if (vacancyId !== undefined) {
      selectQuery = selectQuery.where('ComissionOffices.vacancyId', '=', vacancyId)
      countQuery = countQuery.where('vacancyId', '=', vacancyId)
    }
    if (status !== undefined) {
      selectQuery = selectQuery.where('ComissionOffices.status', '=', status)
    ...
    return {
      ...
      records: records.map((result) => {
        ...
        return comissionOffice
      })
    }
  }

  async create (comissionOfficeData: Omit<CreateComissionOffice, 'createdAt' | 'updatedAt'>) {
    const db = this.db
    const [result] = await db.table('ComissionOffices')
      .insert({
        ...comissionOfficeData,
        createdAt: new Date(),
        updatedAt: new Date()
    ...
    this.logger.info(result, 'ComissionOffice created')
    return result
  }

  async update (id: number, comissionOfficeData: Omit<UpdateComissionOffice, 'updatedAt'>) {
    const db = this.db
    const [result] = await db.table('ComissionOffices')
      .where({ id })
      .update({
