...
const routesPlugin: FastifyPluginAsync = async function routesPlugin (fastify) {
  ...
  const reportsQuerySchema = {
    ...
    properties: {
      ...
      reportNumber: { type: 'string', enum: ['1', '2'], description: 'Filter by report number' },
  ...
  const updateStatusBodySchema = {
    type: 'object',
    properties: {
      status: { type: 'string', enum: ['APPROVED', 'REJECTED'], description: 'New status for the report' }
    },
    required: ['status']
  } as const satisfies JSONSchema
  ...
  server.route({
    ...
    async handler (request, reply) {
      ...
      const currentRecord = await services.reportService().getById(id, {
        includeCycle: true,
        includeStudent: true,
        includeVacancy: true
      })
      ...
      // Update the status
      const updatedRecord = await services.reportService().update(id, { status })

      await services.emailService().sendReportStatusChange({
