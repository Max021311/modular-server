...
const routesPlugin: FastifyPluginAsync = async function routesPlugin (fastify) {
  ...
  server.route({
    ...
    url: '/',
    schema: {
      description: `Endpoint to get final reports with pagination and filters. This endpoint require the user permission \`${PERMISSIONS.VIEW_REPORT}\``,
      tags: ['FinalReports'],
      headers: {
        type: 'object',
        properties: {
  ...
  server.route({
    ...
    url: '/:id',
    schema: {
      description: `Endpoint to update the status of a final report. Only allows changing from PENDING to APPROVED or REJECTED. This endpoint requires the user permission \`${PERMISSIONS.EDIT_REPORT}\``,
      tags: ['FinalReports'],
      headers: {
        type: 'object',
        properties: {
  ...
  server.route({
    ...
    url: '/:id/download',
    schema: {
      description: `Download a final report file. This endpoint requires the user permission \`${PERMISSIONS.VIEW_REPORT}\``,
      tags: ['FinalReports'],
      headers: {
        type: 'object',
        properties: {
